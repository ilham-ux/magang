...
if($buku=="" || $dapat == "") {
	...
	include "../conn.php";
    $jum = 1;
	$query=mysql_query("SELECT * FROM data_buku WHERE id_buku = '$id'");
	$hasil=mysql_fetch_array($query);
	       $sisa = $hasil['jumlah_buku'];
           $tot = $sisa * $jum;
	if ($hasil['jumlah_buku'] == 0) {
		...
		$qu			= mysql_query("UPDATE data_buku SET jumlah_buku=(jumlah_buku-1) WHERE id_buku=$id ");
